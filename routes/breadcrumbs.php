...
// Home > Admin > Airports
Breadcrumbs::for('admin.airports.index', function ($trail): void {
    $trail->parent('admin');
    $trail->push('Airports', route('admin.airports.index'));
});

// Home > Admin > Airports > New
Breadcrumbs::for('admin.airports.create', function ($trail): void {
    $trail->parent('admin.airports.index');
    $trail->push('New', route('admin.airports.create'));
});

// Home > Admin > Airports > [Airport]
Breadcrumbs::for('admin.airports.show', function ($trail, $airport): void {
    $trail->parent('admin.airports.index');
...
Breadcrumbs::for('admin.airports.edit', function ($trail, $airport): void {
...
Breadcrumbs::for('admin.airportLinks.index', function ($trail): void {
    $trail->parent('admin');
    $trail->push('Airport Links', route('admin.airportLinks.index'));
});

// Home > Admin > Airport Links > New
Breadcrumbs::for('admin.airportLinks.create', function ($trail): void {
...
Breadcrumbs::for('admin.airportLinks.edit', function ($trail, $airportLink): void {
...
Breadcrumbs::for('admin.eventLinks.index', function ($trail): void {
    $trail->parent('admin');
    $trail->push('Event Links', route('admin.eventLinks.index'));
});

// Home > Admin > Event Links > New
Breadcrumbs::for('admin.eventLinks.create', function ($trail): void {
...
// Home (no event found)
Breadcrumbs::for('bookings.index', function ($trail): void {
    $trail->parent('home');
});

// Home > [Event]
Breadcrumbs::for('events.show', function ($trail, $event): void {
    $trail->parent('home');
...
Breadcrumbs::for('admin.bookings.edit', function ($trail, $booking): void {
...
Breadcrumbs::for('admin.bookings.autoAssignForm', function ($trail, $event): void {
    $trail->parent('admin.events.show', $event);
    $trail->push('Auto-Assign FL / Route', route('admin.bookings.autoAssignForm', $event));
});

// Home > Admin > [Event] > Route assign
Breadcrumbs::for('admin.bookings.routeAssignForm', function ($trail, $event): void {
    $trail->parent('admin.events.show', $event);
    $trail->push('Route assign', route('admin.bookings.routeAssignForm', $event));
});

// Home > FAQ
Breadcrumbs::for('faq', function ($trail): void {
    $trail->parent('home');
    $trail->push('FAQ', route('faq'));
});

// Home > Admin > FAQ
Breadcrumbs::for('admin.faq.index', function ($trail): void {
    $trail->parent('admin');
    $trail->push('FAQ', route('admin.faq.index'));
});

// Home > Admin > FAQ > New
Breadcrumbs::for('admin.faq.create', function ($trail): void {
    $trail->parent('admin.faq.index');
    $trail->push('New', route('admin.faq.create'));
});

// Home > Admin > FAQ > [FAQ]
Breadcrumbs::for('admin.faq.show', function ($trail, $faq): void {
    $trail->parent('admin.faq.index');
    $trail->push('[' . $faq->id . '] ' . $faq->question);
});

// Home > Admin > FAQ > [FAQ] > Edit FAQ
Breadcrumbs::for('admin.faq.edit', function ($trail, $faq): void {
