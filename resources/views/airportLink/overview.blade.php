    ...
    <table class="table table-hover">
        ...
        @forelse($airportLinks as $airportLink)
            <tr>
                ...
                <td>
                    <form action="{{ route('airportLink.destroy', $airportLink) }}" method="post">
                        @method('DELETE')
        ...
        @empty
            @component('layouts.alert.warning')
                @slot('title')
                    No airport links found
                @endslot
                No airport links are in the system, consider adding one, using the button above
            @endcomponent
        @endforelse
        {{ $airportLinks->links() }}
    </table>
