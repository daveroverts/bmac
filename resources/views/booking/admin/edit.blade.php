                ...
                <div class="card-body">
                    <x-form :action="route('admin.bookings.update', $booking)" method="PATCH">
                        @bind($booking)
                            ...
                            @bind($flight)
                                <x-form-group inline>
                                    <x-form-input name="ctot" :bind="false" value="{{ $flight->ctot?->format('H:i') }}"
                                        type="time" :label="'<i class=\'fa fa-clock\'></i> ' . __('CTOT')">
                                        @slot('append')
                                            z
                                        @endslot
                                    </x-form-input>
                                    <x-form-input name="eta" :bind="false" value="{{ $flight->eta?->format('H:i') }}"
                                        type="time" :label="'<i class=\'fa fa-clock\'></i> ' . __('ETA')">
                                        @slot('append')
                                            z
                                        @endslot
                                    </x-form-input>
                                ...
                                <x-form-input name="oceanicFL" :label="$booking->event->is_oceanic_event
                                    ? __('Oceanic Entry Level')
                                    : __('Cruise FL')">
                                    @slot('prepend')
                                        FL
                                    @endslot
