                                ...
                                <select
                                    ...
                                    @if($eventLink->id)
                                        @foreach($eventLinkTypes as $eventLinkType)
                                            <option
                                                value="{{ $eventLinkType->id }}" {{ old('event_link_type_id', $eventLink->type->id) == $eventLinkType->id ? 'selected' : '' }}>{{ $eventLinkType->name }}</option>
                                        @endforeach
                                    @else
                                        @foreach($eventLinkTypes as $eventLinkType)
                                            <option
                                                value="{{ $eventLinkType->id }}" {{ old('event_link_type_id') == $eventLinkType->id ? 'selected' : '' }}>{{ $eventLinkType->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                                ...
                                @else
                                    <select
                                        ...
                                        @foreach($events as $event)
                                            <option
                                                value="{{ $event->id }}" {{ old('event_id') == $event->id ? 'selected' : '' }}>{{ $event->name }} [{{ $event->startEvent->format('d-m-Y') }}]
                                            </option>
                                        @endforeach
                                    </select>
