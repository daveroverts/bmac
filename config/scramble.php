...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
            BMAC API — provides access to events, bookings, and airports.

            ## Versioning

            The current stable API is **v1**, available at `/api/v1/`.

            ## Deprecated: unversioned routes

            The legacy unversioned routes (`/api/events`, `/api/bookings`, `/api/airports`) are
            **deprecated** and will be removed on **2026-12-31**. All responses from those routes
            include the following headers as per [RFC 8594](https://httpwg.org/specs/rfc8594.html):
